"""Shared fixtures for ``tests/infra/`` (ROADMAP.md 5.4).

Constructing ``TricksyStack`` now reads ``TRICKSY_SES_FROM_ADDRESS``/
``TRICKSY_SES_DOGFOOD_RECIPIENTS`` from the environment to build its SES identities, the same way
``tests/conftest.py``'s ``table`` fixture keeps the storage tests independent of a real AWS
account: fake-but-valid-looking addresses here, never anything real committed to this (public)
repo.
"""
