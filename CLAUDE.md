...
Tricksy: a server-authoritative, asynchronous server for trick-taking games, played over minutes
or hours rather than in real time. One game ships, **Texas 42** - the partnership domino game,
including the nello, plunge and sevens special contracts - and it lives at
`tricksy.games.texas42`. The MVP ships a CLI client, but the architecture exists so that a web,
mobile or chatbot client is later a thin client against the same API, not a rewrite.

The project was called `t42` until the rename to `tricksy`; the rules engine moved from
`t42.engine` to `tricksy.games.texas42` at the same time. `GAME_KIND` stayed `"texas42"` - that
value names the game, not the project, and it is written to disk and returned on the wire. Nothing
dispatches on it: there is still no `Game` protocol, no registry, and no per-kind branch anywhere
(DESIGN.md §11.1).
...
1.1 (item shapes and codec) is complete: `tricksy.storage.codec` encodes and decodes `GameState`,
...
wired-in), so `tricksy.storage.events` supplies the write direction: `event_for_move`/
`hand_dealt_event`/`events_for_move` translate an accepted move (and any deal it triggers) into
the `Event`(s) it produces. `tricksy.storage.replay.replay(game_id, players, config, events)` is the
...
1.3 (repository writes) is complete: `tricksy.storage.repository` is the first real boto3 dependency.
...
`VersionConflict` (`tricksy.storage.errors`, alongside `GameNotFound`/`GameAlreadyExists`) - the
...
`create_game`/`append` and confirms `tricksy.storage.replay.replay` over the resulting event log
...
`Event` types, `apply_move` and `tricksy.storage.replay` stay untouched - idempotency is a repository
concern only (invariant 1).

1.5 (player-specific view) is complete: `tricksy.games.texas42.projection.project(state, player_id)` is the
...
deliberately not shared with `tricksy.storage.codec`: `tricksy.games.texas42` may not import from `tricksy.storage`
(invariant 1), and the two serve different purposes - a durable wire format versus a client
read-model. Proven in `tests/games/texas42/test_projection.py` by a leakage test that drives real games
...
the whole test session and `real_table` creates/drops a fresh `Tricksy` table in it per test - the
...
- **Identity** (§6.1, §12): username + password minting **per-device bearer tokens**, hashed with
  ...
  an `email` field, so Phase 4 adds a branch rather than a migration. `tricksy/storage/accounts.py`.
- **The lobby** (§4.1): a game is created `WAITING` with only its creator seated, and the join
  ...
  entirely in `tricksy/storage/lobby.py`; the engine still has no notion of a partially seated game.
...
- **A saved rule set is a copy, not a reference** (§5.1). `tricksy/storage/rule_sets.py` stores named
...
- **Invites are a permission grant, not a seat reservation** (§6.2). `tricksy/storage/invites.py` is
  ...
  can't import back). `tricksy/api/app.py`'s invite handler does that validation itself, the same
...
- **3.0** closed the gap the CLI's command set surfaces before any CLI code exists: an invite's
  player id is handed back once, in a response no client keeps, so `tricksy uninvite` had nothing to
  address a revocation to. `invites.list_invites_for_game` plus `GET /games/{game_id}/invites`
  (seated callers only) fix that, giving `tricksy invited`/`tricksy uninvite` a finished surface.
- **3.1 (skeleton, profiles, credentials)** is complete: `src/tricksy/cli/main.py`'s `main(argv) -> int`
  ...
  `config.py` holds `~/.config/tricksy/config.json` (honouring `XDG_CONFIG_HOME`), written `0600`
...
- **3.3 (rendering)** is complete: `render.py` is pure `dict -> str` - no HTTP, no `argparse`, and
  (like every module under `tricksy.cli`) no import from `tricksy.games`, `tricksy.storage` or boto3. It
  therefore keeps its own seat- and suit-name tables rather than importing `Seat`/`Suit`, the
  duplication DESIGN.md §7 explicitly trades for `tricksy.cli` being provably just a client. Its most
  load-bearing piece is `render_legal_moves`: it renders each entry of `view["legal_moves"]` as the
  literal `tricksy ...` command that would submit it, which is formatting the server's own answer, not
...
- **3.4 (account and table commands)** is complete: the thirteen commands from DESIGN.md §7 that
  ...
  `build_client`/`emit` turn `--profile`/`TRICKSY_PROFILE` into an authenticated `ApiClient` and give
  ...
  `handler`) is what `tricksy.cli.commands.COMMANDS` is built from, split out from `main.py` so the
...
- **3.6 (running it locally)** is complete, and is the one piece of non-CLI code in the phase: the
  `Tricksy` table definition, previously reachable only through a pytest fixture, is now
  `src/tricksy/storage/schema.py`'s `create_table(dynamodb, name)`, plus a
  `python -m tricksy.storage.schema` entry point for creating it outside a test run. The `table` (moto)
  ...
  was missing the `OpenGames` GSI. It lives under `tricksy.storage`, not a `tricksy dev` subcommand,
  because `tricksy.cli` may not import boto3 - the layering rule 3.7 checks by test.
- **3.7 (tests)** is complete, closing out Phase 3. `tests/cli/test_render.py` gained **the third
  ...
  `tests/cli/test_layering.py` is a static, `ast`-based check that no module under `tricksy.cli`
  imports `tricksy.games`, `tricksy.storage` or boto3 - the claim three module docstrings already made,
...
- **4.1 (the send channel)** is complete: new top-level package `src/tricksy/notifications/`, importing
  only `boto3` (nothing from `tricksy.storage` or `tricksy.games.texas42` yet - that permission is reserved for
  ...
  same way `tricksy.api.deps`'s DynamoDB resource is, avoiding a dependency on `sesv2` stubs), and
  ...
  `get_sender()` picks between them via `TRICKSY_EMAIL_SENDER`, mirroring `tricksy.api.deps`'s env-var
  ...
  the same property `tricksy/cli/render.py` has for the same reason. Nothing called these yet at the
...
- **4.2 (contact channels and email verification)** is complete: `ContactChannel` gains
  ...
  never be replayed. This is `tricksy.notifications`'s first real caller: `POST
...
- **4.4 (Streams and the local pump)** is complete: `schema.create_table` enables the table's
  ...
  tricksy.notifications.pump`) discovers the stream's shards, tracks a `TRIM_HORIZON` iterator per open
  ...
  modules this phase adds, is an `ast`-based check that nothing under `tricksy.notifications` imports
  `tricksy.games` or any of `tricksy.storage`'s `repository`/`codec`/`replay` (only `accounts` is allowed).
...
- **4.5 (the handler)** is complete: `handler.py`'s stub is replaced by a pure classifier,
  ...
  `TRICKSY_EMAIL_SENDER=console` - the console print of "It's your turn" for the correct seat, with
...
- **4.6 (CLI commands)** is complete: `tricksy contacts`, `tricksy contact add|remove|verify|confirm|
  mute|unmute`, `tricksy forgot-password` and `tricksy reset-password` join `src/tricksy/cli/commands/
  ...
  `tricksy whoami` silently stale the moment `tricksy contact mute` existed. `tests/cli/_helpers.py`'s
  ...
  mismatch. `tests/cli/conftest.py`'s `cli_app_client` now also overrides `tricksy.notifications.
...
- **4.7 (tests)** is complete, closing out Phase 4. Every other test the phase named had already
  ...
  real stream through `tricksy.notifications.pump.poll` into `send_notifications` - not a spy standing
...
- **`tricksy.games.texas42.GAME_KIND`** (`"texas42"`) is written as `kind` on `GAME#/META`, `GAME#/STATE`,
...
- **`tricksy.storage.lobby` no longer derives its table size from the engine.** `seat_count` lives on
  ...
  engine. This was the only hard four-handed assumption outside `tricksy.games.texas42`.
- **`META.scores` replaced `META.marks`** as an open `{label: int}` map keyed on `team.name.lower()`
  ...
  `handler._read_scores` and `messages.render_game_over` iterating it, **`tricksy.notifications` now
  encodes nothing about how any one game scores**, on top of already importing nothing from
  `tricksy.games.texas42`.
...
Two things documented rather than changed. `tricksy.storage.replay` reconstructs a game by feeding
...
`legal_moves`/`project`, a codec quintuple, and `events_for_move` - everything `tricksy.storage` does
...
```
src/tricksy/games/    where a game's rules engine lives. One game ships; nothing dispatches on
                    which, and this directory is a name rather than an abstraction (DESIGN.md §11.1)
src/tricksy/games/texas42/     pure rules library (Phase 0 - complete)
...
src/tricksy/storage/    DynamoDB event log + materialized state   (Phases 1, 2 and 2.7, complete)
    ...
    schema.py       create_table(dynamodb, name); `python -m tricksy.storage.schema` (3.6);
                     stream enabled, NEW_AND_OLD_IMAGES (4.4)
src/tricksy/api/        FastAPI app behind Mangum                 (Phases 2, 2.7 and 4.2, ongoing)
...
src/tricksy/cli/        thin command-line client                  (Phase 3, complete)
    main.py         argparse + dispatch; `main(argv) -> int` returns rather than raises (3.1)
    config.py       ~/.config/tricksy/config.json, named profiles, 0600 (3.1)
...
src/tricksy/notifications/  the send channel (Phase 4, complete). Imports nothing from tricksy.games and,
    ...
    pump.py         poll()/main(); `python -m tricksy.notifications.pump` polls the stream
...
tests/games/texas42/       mirrors the engine modules; test_full_game.py is the Phase 0 milestone demo;
...
tests/storage/      mirrors src/tricksy/storage/; _helpers.py's `started_game` reaches a dealt game
...
tests/cli/          mirrors src/tricksy/cli/; conftest.py's `cli_app_client` (`TestClient(app)` + the
                    moto `table` override) and `_config_home` are shared by every file here.
                    _helpers.py's `FakeTransport` fakes `tricksy.cli.api.Transport` with no network,
...
tests/notifications/  mirrors src/tricksy/notifications/; _helpers.py's `FakeSender` records sends
                    ...
                    `tricksy.notifications`-specific ast import guard; test_handler.py (4.5) covers
...
1. **The engine is pure.** Nothing under `tricksy.games` may do I/O, import boto3, or import from
   `tricksy.storage`, `tricksy.api` or `tricksy.cli`. It takes a state plus a proposed move and
   returns a new state or raises. Randomness is injected (`rng: Random`), never ambient. Stated
   over the whole `games` package, not just `texas42`, so a second engine inherits the rule.
